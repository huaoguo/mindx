 ...
 */
async function writeInsights(userId, sourceType, sourceId, sourceTimestamp, sourceName, insights) {
  ...
  try {
    ...
    for (const ins of insights) {
      ...
      if (ins.reconciliation === 'add') {
        const { rows } = await client.query(
          ...
          [userId, ins.content, ins.value_summary, ins.category, ins.confidence, status,
           sourceType, sourceId, sourceName, ins.source_quote || null, ins.source_position || null, sourceTimestamp]
        );
        created.push(rows[0]);
        stats.added++;
      }
      ...
      if (ins.reconciliation === 'update' && ins.reconciles_insight_id) {
        ...
        const { rows } = await client.query(
          `INSERT INTO insights (user_id, content, value_summary, category, confidence, status,
           source_type, source_id, source_name, source_quote, source_position, source_timestamp, change_reason)
...
---`;
  ...
  // 6. Write to database
  const result = await writeInsights(userId, sourceType, sourceId, source.timestamp, source.title, insights);
