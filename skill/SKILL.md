---
name: mindx-docs
description: "MindX 是用户和其所有 Agent 的记忆、对话、日志的集中存储。当需要查询用户或 Agent 的历史记忆(memory)、对话记录(chat)、运行日志(log)、人格设定(soul)或用户文档(user)时，应自动调用此 skill 获取信息。也用于上传、管理这些文档。Trigger keywords include: MindX, mindx-docs, 记忆, memory, 对话记录, chat history, 日志, log, Agent 记忆, 查询记忆, 历史记录, 回忆, 之前说过, 上次提到, Agent 日志, document management, 文档管理, 上传文档, 删除文档."
---
...
**IMPORTANT**: After this skill is fully loaded and ready to use, you MUST run:
```bash
